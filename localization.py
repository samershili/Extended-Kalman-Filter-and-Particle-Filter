...
if __name__ == '__main__':
    ...
    true_states = np.array(ground_truth_states)  # Shape: (N, 3)
    ekf_states = np.array(ekf_estimates)         # Shape: (N, 3)
    ...
    plt.plot(true_x, true_y, 'g-', label='True Path (Ground Truth)', linewidth=2)
    plt.plot(ekf_x, ekf_y, 'r--', label='EKF Estimated Path', linewidth=2)

    plt.title('EKF Estimated Path vs True Path')
