...
class Field:
    ...
    def G(self, x, u):
        ...
        # Partial derivatives of x', y', theta' with respect to x, y, theta
        G = np.zeros((3, 3))
    
        # dx'/dx, dy'/dy, dtheta'/dtheta = 1
        G[0, 0] = 1
        G[1, 1] = 1
        G[2, 2] = 1
    
        # dx'/dtheta = -trans * sin(theta + rot1)
        G[0, 2] = -trans * np.sin(prev_theta + rot1)
        # dy'/dtheta = trans * cos(theta + rot1)
    ...
    def V(self, x, u):
        ...
        # Partial derivatives of x', y', theta' with respect to rot1, trans, rot2
        V = np.zeros((3, 3))
        
        # dx'/drot1 = -trans * sin(theta + rot1)
        V[0, 0] = -trans * np.sin(prev_theta + rot1)
        
        # dx'/dtrans = cos(theta + rot1)
        V[0, 1] = np.cos(prev_theta + rot1)
        
        # dy'/drot1 = trans * cos(theta + rot1)
        V[1, 0] = trans * np.cos(prev_theta + rot1)
        
        # dy'/dtrans = sin(theta + rot1)
    ...
    def H(self, x, marker_id):
        ...
        # Jacobian for bearing observation
        H = np.zeros((1, 3))
